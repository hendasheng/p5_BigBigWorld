...
function draw() {
  ...
  for (let gy = 0; gy < bgRows; gy++) {
    ...
    for (let gx = 0; gx < bgCols; gx++) {
      ...
      const interval = clampInt(Math.floor(lerp(34, 10, influence)), 10, 34);
      ...
      if (
        ...
        if (curIdx !== fillIndex[si]) {
          fillCooldown[si] = clampInt(Math.floor(lerp(28, 10, influence)), 10, 28);
          activityCount += 1;
        }
      }
      fillIndex[si] = curIdx;
      ...
      const group = indexToGroup(ri);
      const gs = fillRampGroups[group];
      // Explicit left-drift phase: strong in wake, weak elsewhere.
      const drift = Math.floor(t * (0.4 + 4.2 * tailCone * motionGain * contactGain));
      const pick = clampInt(
        Math.floor(hash01((gx + drift) * 31 + ri, gy * 17 + ri, group + 101 + drift) * gs.length),
        0,
        gs.length - 1
      );
      const ch = gs[pick] ?? " ";
  ...
  if (showHud) {
    drawHud(moveSpeed, contactStrength, activityCount);
  }
...
function drawHud(moveSpeed, contactStrength, activityCount) {
  fill(0, 170);
  rect(12, 12, 620, 64, 8);
  fill(255);
  textSize(12);
  text(
    `H: HUD  speed:${moveSpeed.toFixed(2)}  contact:${contactStrength.toFixed(2)}  changes:${activityCount}`,
    20,
    20
  );
}
