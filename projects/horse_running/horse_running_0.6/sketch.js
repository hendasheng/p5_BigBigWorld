...
function setup() {
  ...
  if (window.Tweakpane && !pane) {
    ...
    pane.addInput(params, "horseSize", {
      label: "horse size",
      min: 0.5,
      max: 2.2,
      step: 0.01,
...
function draw() {
  ...
  const horseSizeScale = constrain(params.horseSize, 0.5, 2.2);
  textSize(fontPx * horseSizeScale);
  textFont("JetBrains Mono, monospace");
  drawingContext.font = `${textSize()}px "JetBrains Mono", monospace`;
  drawingContext.textBaseline = "top";

  let drawSize = fontPx * horseSizeScale;
  ...
  if (blockW > width || blockH > height) {
    ...
    drawSize = fontPx * horseSizeScale * fit;
    textSize(drawSize);
    textFont("JetBrains Mono, monospace");
    drawingContext.font = `${textSize()}px "JetBrains Mono", monospace`;
