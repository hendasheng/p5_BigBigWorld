...
function draw() {
  ...
  // text(`第 ${paragraphIndex + 1} 段 | ${currentLanguage.toUpperCase()}`, width - 20, height - 20);
  // pop();
}
...
function initChars(txt) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let j = 0; j < line.length; j++) {
      let char = line[j];
      let xPos = lineBaseX + textWidth(line.substring(0, j));
      chars.push({
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let j = 0; j < line.length; j++) {
      if (charIndex < chars.length) {
        ...
        c.startX = c.x; 
        c.startY = c.y;
        c.targetX = targetXBase + textWidth(line.substring(0, j));
        c.targetY = yPos;
        c.targetChar = line[j];
      }
      charIndex++;
...
function setupTweakpane() {
  ...
  textFolder.addInput(params, 'fontSize', { label: '字体大小', min: 12, max: 120, step: 1 })
    .on('change', (ev) => {
      tSize = ev.value;
  ...
  animFolder.addInput(params, 'totalDuration', { label: '动画时长', min: 500, max: 10000, step: 100 })
    .on('change', (ev) => totalDuration = ev.value);
  animFolder.addInput(params, 'minScale', { label: '最小缩放', min: 0.1, max: 2.0, step: 0.1 });
